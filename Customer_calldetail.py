...
class Util:
    ...
    def parse_customer(self,list_of_customers,list_of_calls):
        for i in range(len(list_of_customers)):
            list_of_customers[i].list_of_calls=[]
            for index in range(len(list_of_calls)):
                if list_of_customers[i].phone_no==list_of_calls[index].phone_no:
                    list_of_customers[i].list_of_calls.append(list_of_calls[index])
